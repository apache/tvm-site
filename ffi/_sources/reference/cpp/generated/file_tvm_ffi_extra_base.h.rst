
.. _file_tvm_ffi_extra_base.h:

File base.h
===========

|exhale_lsh| :ref:`Parent directory <dir_tvm_ffi_extra>` (``tvm/ffi/extra``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS



Base header for Extra API. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``tvm/ffi/extra/base.h``)
-------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_tvm_ffi_extra_base.h.rst



Detailed Description
--------------------

The extra APIs contains a minmal set of extra APIs that are not required to support essential core functionality. 




Includes
--------


- ``tvm/ffi/c_api.h`` (:ref:`file_tvm_ffi_c_api.h`)



Included By
-----------


- :ref:`file_tvm_ffi_extra_deep_copy.h`

- :ref:`file_tvm_ffi_extra_json.h`

- :ref:`file_tvm_ffi_extra_module.h`

- :ref:`file_tvm_ffi_extra_serialization.h`

- :ref:`file_tvm_ffi_extra_structural_equal.h`

- :ref:`file_tvm_ffi_extra_structural_hash.h`



