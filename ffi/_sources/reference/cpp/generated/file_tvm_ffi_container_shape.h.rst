
.. _file_tvm_ffi_container_shape.h:

File shape.h
============

|exhale_lsh| :ref:`Parent directory <dir_tvm_ffi_container>` (``tvm/ffi/container``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS



Container to store shape of an Tensor. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``tvm/ffi/container/shape.h``)
------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_tvm_ffi_container_shape.h.rst





Includes
--------


- ``algorithm``

- ``ostream``

- ``tvm/ffi/container/array.h`` (:ref:`file_tvm_ffi_container_array.h`)

- ``tvm/ffi/error.h`` (:ref:`file_tvm_ffi_error.h`)

- ``tvm/ffi/type_traits.h`` (:ref:`file_tvm_ffi_type_traits.h`)

- ``utility``

- ``vector``



Included By
-----------


- :ref:`file_tvm_ffi_container_tensor.h`




Namespaces
----------


- :ref:`namespace_tvm`

- :ref:`namespace_tvm__ffi`


Classes
-------


- :ref:`exhale_class_classtvm_1_1ffi_1_1Shape`

- :ref:`exhale_class_classtvm_1_1ffi_1_1ShapeObj`

