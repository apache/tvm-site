
.. _file_tvm_ffi_container_container_details.h:

File container_details.h
========================

|exhale_lsh| :ref:`Parent directory <dir_tvm_ffi_container>` (``tvm/ffi/container``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS



Common utilities for typed container types. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``tvm/ffi/container/container_details.h``)
------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_tvm_ffi_container_container_details.h.rst





Includes
--------


- ``sstream``

- ``string`` (:ref:`file_tvm_ffi_string.h`)

- ``tvm/ffi/any.h`` (:ref:`file_tvm_ffi_any.h`)

- ``tvm/ffi/memory.h`` (:ref:`file_tvm_ffi_memory.h`)

- ``tvm/ffi/object.h`` (:ref:`file_tvm_ffi_object.h`)

- ``type_traits`` (:ref:`file_tvm_ffi_type_traits.h`)

- ``utility``



Included By
-----------


- :ref:`file_tvm_ffi_container_array.h`

- :ref:`file_tvm_ffi_container_map.h`

- :ref:`file_tvm_ffi_container_variant.h`




Namespaces
----------


- :ref:`namespace_tvm`

- :ref:`namespace_tvm__ffi`

